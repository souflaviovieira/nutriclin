      ...
      <div className="bg-white w-full sm:w-[210mm] min-h-[297mm] shadow-2xl p-8 sm:p-[15mm] relative mb-20 animate-in zoom-in-95 duration-500">
        ...
        <header className="flex justify-between items-start border-b-2 border-slate-100 pb-8 mb-8">
          <div className="flex items-center gap-4">
            {profile?.logo_url ? (
              <img src={profile.logo_url} className="h-16 w-auto object-contain" alt="Logo Clínica" />
            ) : (
        ...
        <div className="lg:col-span-4 space-y-3">
          ...
          {DELIVERABLES.map((item) => (
            <button
              ...
              className={`w-full flex items-center gap-4 p-4 rounded-2xl border transition-all text-left group ${activeType === item.id
            ...
            >
              <div className={`p-3 rounded-xl transition-all ${activeType === item.id ? item.color : 'bg-white text-slate-300'
                }`}>
                {item.icon}
              </div>
